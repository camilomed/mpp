...
//		listPerson.stream().filter(p-> p instanceof Staff || p instanceof StaffStudents || p instanceof Faculty).map(null);
	return	listPerson.stream().mapToDouble(Person::getSalary).sum();
//		double salary =0.0;
...
//		System.out.println("name : "+ );
	...
	public void unitsPerFaculty() {
		...
		listPerson.stream().filter(p -> p instanceof Faculty).map(p-> (Faculty) p).
		forEach(p-> System.out.println("type: "+p.getName()+"/b "+"ubits per Faculty "+p.getTotalUnits()));;
