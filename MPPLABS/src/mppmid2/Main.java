...
public class Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		customerSmallestPoints(null);
		
		
	}
	
	public static void getBudgetMilitay(List<ICustomer> militaryCustomer) {
		
		for(ICustomer custumer: militaryCustomer) {
			
			if (custumer instanceof MilistaryCustomer) {
				
				
				
				System.out.println(((ACustomer) custumer).getName()+ ((MilistaryCustomer) custumer).getMaximumSpend());
			}
		}
	...
	public static void customerSmallestPoints(List<ICustomer> listCustomer) {
		 double smallCustomer1 = ((ACustomer) listCustomer).getListOrder().get(0).getListOrderLine().get(0).getPoints();
		 double LiCustomer = ((OrderLine) listCustomer).getPoints();
		 System.out.println("first "+smallCustomer1+" second "+LiCustomer);
		for(ICustomer customer: listCustomer) {
			ACustomer A = (ACustomer) customer;
			double smallCustomer = A.getListOrder().get(0).getListOrderLine().get(0).getPoints();
			for(Order order:( (ACustomer) customer).getListOrder()) {
				for(OrderLine orderline : order.getListOrderLine()) {
				if(smallCustomer>	orderline.getPoints()) {
					smallCustomer = orderline.getPoints();
				}
			...
			System.out.println("name "+ ((ACustomer) customer).getName()+ "ponts "+smallCustomer);
		}
