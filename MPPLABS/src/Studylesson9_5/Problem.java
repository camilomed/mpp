...
public class Problem {
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		//implement
		return list.stream().sorted((x,y)->x.getZip().compareTo(y.getZip())).filter(x->!x.getCity().contains("e")).filter(x->x.getCity().length()>=6).map(Customer::getZip).distinct().collect(Collectors.toList());
