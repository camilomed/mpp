...
public class Main {
	...
	private void showAllOrderItems() {
		System.out.println("\n==============\nAll order items:"+
		orders.stream().map(x->x.getOrderItems()).collect(Collectors.toList()));
System.out.println("\n==============\nAll order items:"+
		orders.stream().flatMap(x->x.getOrderItems().stream()).collect(Collectors.toList()));

	orders.stream().map(Order::getOrderItems).flatMap(List::stream).forEach(System.out::print);;
		//implement
	}
